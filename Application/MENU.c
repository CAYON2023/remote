...
#define MENU_LINE_H 20 // 行高
#define MENU_PADDING 3 // 内边距
#define MENU_MARGIN 3  // 外边距
...
#define ANIMATION_SPEED 0.35 // 动画速度 0 < 范围 <= 1;
...
uint8_t Key_GetEvent_Down(void)
{
    uint8_t temp_bool = 0;
    if (key.key_down == 1)
    {
...
uint8_t Key_GetEvent_Right(void)
{
    return 0;
}
...
int menu_command_callback(enum _menu_command command, ...)
{
    ...
    case SHOW_STRING: // 参数:( int16_t x, int16_t y, char *str ); 返回: uint8_t 字符串长度;
    {
        ...
        /* 按需使用参数 */
        OLED_PrintASCIIString(show_x, show_y, show_string, &afont16x8, 0); // 显示字符串
...
void MENU_Information(void)
{
    ...
    menu_command_callback(SHOW_STRING, 5, 0, "R6-CAR");
    menu_command_callback(SHOW_STRING, 5, 20, "V0.1 Beta");
    menu_command_callback(SHOW_STRING, 5, 40, "By CAYON");
